    ...
    public async Task SyncFromReport(Guid id, string reportFolder)
    {
        ...
        await Parallel.ForEachAsync(values, async (value, _) =>
        {
            ...
            try
            {
                ...
                // Sync code info
                Result result = await cxxAggregate.UpdateUserDescription(id, codeInfo.NameSpace, codeInfo.ClassName, codeInfo.Spelling, codeInfo.CxType, codeInfo.Description);
                if (result.IsFailed)
