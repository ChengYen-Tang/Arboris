...
[TestClass]
public class MultipleProjectsShareHeadersContext2Tests
{
    ...
    public MultipleProjectsShareHeadersContext2Tests()
    {
        string testDataFolder = Path.Combine(AppContext.BaseDirectory, "Core", "Analyze", "TestData");
        string solutionPath = Path.Combine(testDataFolder, "Context2");
    ...
    public async Task TestMemberLinked()
    {
        ...
        Guid member1 = await dbContext.Cxx_Nodes.Where(item => item.VcProjectName == "Online_Motion" && item.Spelling == "run" && item.CursorKindSpelling == "CXXMethod").Select(item => item.Id).FirstOrDefaultAsync();
        ...
        Guid[] membersId = await dbContext.Cxx_NodeMembers
            ...
            .ToArrayAsync();
        Assert.AreEqual(3, membersId.Length, "Expected 2 members linked to the class node");
    ...
    public async Task TestDependencyLinked()
    {
        ...
        Guid dllMain1 = await dbContext.Cxx_Nodes
            .Where(item => item.VcProjectName == "Context2" && item.Spelling == "DllMain" && item.CursorKindSpelling == "FunctionDecl")
            .Select(item => item.Id)
            .FirstOrDefaultAsync();
        ...
        Guid[] dependencies2 = await dbContext.Cxx_NodeDependencies
            ...
            .Select(item => item.NodeId)
            .ToArrayAsync();
        Assert.AreEqual(1, dependencies2.Length, "Expected 1 dependency for member 'run' in 'Offline_Motion' project");
